...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
      ...
      <div className="flex h-[72px] items-center px-6 border-b border-border/50 shrink-0">
        <div className="flex items-center gap-3">
          <div className="relative group/logo">
            ...
            <div className="relative flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-primary to-blue-700 border border-primary/30 shadow-md shrink-0 overflow-hidden">
          ...
          <div className="leading-tight">
            <p className="text-[16px] font-extrabold text-slate-800 tracking-tight">COTIZADOR</p>
            <p className="text-[9px] text-slate-400 tracking-[0.2em] uppercase font-bold">FYM TECH</p>
          </div>
        </div>
      </div>
        ...
        {navGroups.map((group, idx) => {
          ...
          return (
            <div key={idx} className="space-y-1">
              <p className="px-3 mb-2 text-[10px] font-bold uppercase tracking-[0.15em] text-slate-400 flex items-center gap-2">
                {group.title}
              </p>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className={`flex h-8 w-8 items-center justify-center rounded-lg transition-all duration-300 ${
                        isActive ? 'bg-primary/15 text-primary' : 'bg-slate-100 text-slate-400 group-hover:text-slate-600'
                      }`}>
      ...
      <div className="shrink-0 p-4 border-t border-border/50 bg-muted/30">
        <div className="group/user flex items-center gap-3 rounded-2xl p-2.5 bg-muted/40 border border-border/40 transition-all hover:bg-muted/60">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[13px] font-bold text-slate-800 truncate tracking-tight">{user.fullName}</p>
          ...
          <button
            onClick={logout}
            className="p-2 rounded-lg text-slate-400 hover:text-rose-500 hover:bg-rose-50 transition-all duration-200"
            title="Cerrar sesión"
          >
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-background font-jakarta">
      {sidebarOpen && (
        <div className="fixed inset-0 z-40 bg-slate-900/40 backdrop-blur-sm md:hidden animate-in fade-in duration-300" onClick={() => setSidebarOpen(false)} />
      )}

      {/* Mobile sidebar */}
      <aside className={`fixed inset-y-0 left-0 z-50 w-[280px] flex-col bg-white md:hidden flex transition-transform duration-500 cubic-bezier(0.4, 0, 0.2, 1) ${
        sidebarOpen ? 'translate-x-0 shadow-2xl' : '-translate-x-full'
      }`}>
        <button
          onClick={() => setSidebarOpen(false)}
          className="absolute right-4 top-5 p-2 text-slate-500 hover:text-slate-800 rounded-xl hover:bg-slate-100 transition-colors"
        >
          <X className="h-5 w-5" />
        </button>
        <SidebarContent />
      ...
      {/* Desktop sidebar */}
      <aside className="hidden w-[280px] flex-col bg-white md:flex border-r border-border relative z-20 shadow-[1px_0_0_0_hsl(var(--border))]">  
        <SidebarContent />
      </aside>
      ...
      <div className="flex flex-1 flex-col overflow-hidden relative">
        ...
        {/* Mobile topbar */}
        <header className="flex h-[60px] items-center justify-between border-b border-border bg-white/90 backdrop-blur-md px-4 md:hidden shrink-0 z-30">
          <div className="flex items-center gap-3">
            <button
            ...
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-sm font-black text-slate-800 tracking-tight">COTIZADOR</span>
            </div>
          </div>
